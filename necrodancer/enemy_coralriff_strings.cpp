...
enemy_coralriff_strings::enemy_coralriff_strings()
{
}
...
HRESULT enemy_coralriff_strings::init(string imgName, int idxX, int idxY)
{
...
void enemy_coralriff_strings::release()
{
}

void enemy_coralriff_strings::update()
{
...
void enemy_coralriff_strings::render()
{
	if (OBJECTMANAGER->getBossAlive())
	{
		if (_heart > 0)
		...
		else
		{
			if (_isSaw)
			{
				//IMAGEMANAGER->findImage("enemy_shadow")->render(_posX - 24, _posY - 26 + _posZ);
				if (!_isLeft)
					_img->aniRenderReverseX(_posX, _posY, _ani);
				else
					_img->aniRender(_posX, _posY, _ani);
			}
		}
...
void enemy_coralriff_strings::aniSetUp()
{
	KEYANIMANAGER->addAnimationType("enemy_coralriff_strings");

	int stand[] = { 0,1,2,3 };
	KEYANIMANAGER->addArrayFrameAnimation("enemy_coralriff_strings", "coralriff_strings_stand", "enemy_coralriff_strings", stand, 4, ANISPEED, true);

	int stand_shadow[] = { 4,5,6,7 };
	KEYANIMANAGER->addArrayFrameAnimation("enemy_coralriff_strings", "coralriff_strings_stand_shadow", "enemy_coralriff_strings", stand, 4, ANISPEED, true);

	_ani = KEYANIMANAGER->findAnimation("enemy_coralriff_strings", "coralriff_strings_stand");
	_ani->start();
}

void enemy_coralriff_strings::aniPlay_Stand()
{
	if (_isSaw)
	{
		//그림자이미지
		if (_isSight)
		{
			...
			{
				_ani = KEYANIMANAGER->findAnimation("enemy_coralriff_strings", "coralriff_strings_stand_shadow");
				_ani->start();
			}
		}
		else
...
void enemy_coralriff_strings::hitEnemy(int damage)
{
	soundHit();
	_isDrawHP = true;
	_heart -= damage;
	...
	if (_heart <= 0)
	{
		...
		SOUNDMANAGER->setStringsVolume("boss_strings", _volume);
